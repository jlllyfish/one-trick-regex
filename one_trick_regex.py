...
with col1:
    ...
    regex_pattern = st.text_input("Entrez votre expression régulière:", 
                              ...
                              help="Exemple: ^\d{2}-\d{2}-\d{4}$ pour valider une date au format JJ-MM-AAAA")
    ...
    if test_button:
        try:
            ...
            if results:
                ...
                # Appliquer le style
                styled_df = df.style.applymap(highlight_match, subset=['Correspond'])
...
# Pied de page avec exemples courants
st.markdown("---")
